# Prova de recuperació — Matemàtiques 1r i 2n d'ESO

Lloc estàtic per muntar l'examen global de recuperació d'un alumne que arrossega
les matemàtiques d'un curs anterior. HTML, CSS i JavaScript vainilla: cap build,
...
**Nivell.** Canvia el pes de les tres dificultats del banc. `Mínims` carrega a
nivell 1 i 2 i deixa el 3 gairebé fora, que és el que sol tocar en una
recuperació; `Exigent` fa el contrari.
...
**Les icones de cada pregunta** (apareixen en passar-hi el ratolí):
`☆` la fixa perquè sobrevisqui a *Altres preguntes*, `⟳` la canvia per una altra
del mateix contingut, `↑ ↓` la mou, `✕` la treu.
...
```
El .docx del departament  →  l'estructura: 38 sabers, amb les seves hores
repas (banc de 892 items) →  les preguntes de la prova i les seves solucions
llibre (296 PDFs)         →  què repassar, al pla de repàs
Mates amb Bogdan (12 PDF) →  material d'ampliació, al pla de repàs
```

Del banc de `repas`, **411 ítems** queden assignats a algun saber de 1r o 2n.
...
**1r d'ESO té forats reals** i l'eina no els amaga: els sabers sense banc surten
en gris amb l'etiqueta *sense preguntes al banc*, i no es poden marcar.

| Saber de 1r sense banc | Hores |
|---|---|
| Comprensió lectora matemàtica | 12 |
| Arrel quadrada | 3 |
| Elements geomètrics al pla | 3 |
| Magnituds i unitats | 2 |
| Llenguatge algebraic | 3 |

I tres més que en tenen molt poc: Angles (1 ítem), Perímetres (2), Polígons (3).
Per a aquests, o bé fas servir **+ Pregunta pròpia**, o bé s'escriuen generadors
nous a `repas` (`tools/c_<tema>.py`) i es torna a compilar.
